...
type Store interface {
	...
	FetchUser(userID string) (*models.User,error)
	FetchUserWithPassword(email string)(*models.User,error)
...
func initJWTKeys() *rsa.PrivateKey {
	...
	f, err := os.Open("/etc/boiler/jwt.key")
	if err != nil {
		if os.IsNotExist(err) {
			...
			err = ioutil.WriteFile("/etc/boiler/jwt.key", keyBytes, os.ModeType)
			if err != nil {
